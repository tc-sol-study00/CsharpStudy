...
namespace OigGsSimulationToObjectwithInterface {
    /// <summary>
    /// GSシミュレーション　インターフェース使用＋合成利用（継承なし）
    /// </summary>
    class Programs {
        ...
        static void Main() {
            ...
            foreach (IJidoSha aCar in JidoShas) {
                ...
                Console.WriteLine("{0}のシミュレーション", aCar.CarAttribute.CarName);
                ...
                while (true) {
                    ...
                    if (!oilStatus) {
                        Console.WriteLine("オイル交換必要です。走行距離={0:F2}, 次回交換時期={1:F2} ", ((JidoShawithOil)aCar).OilChangeManage.SouSoukouKyori, ((JidoShawithOil)aCar).OilChangeManage.SouSoukouKyoriAtNextOilChange);
                        flgDisplayed = true;
                    }
                    ...
                    if (strFuelWillness == "y") {   //GSに行くと指示された
                        ...
                        if (aCar is JidoShawithOil jidoShaWithOil2) {
                            ...
                            if (!jidoShaWithOil2.OilCheck()) {
                                ...
                                //オイル交換
                                gsStation.OilCharge(jidoShaWithOil2.OilChangeManage, jidoShaWithOil2.CarAttribute);
                            }
                        }
                        //家に戻る
